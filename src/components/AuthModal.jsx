      ...
      <div className="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-soft animate-slide-up">
        ...
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display text-lg font-bold text-ink-900">
              {step === "phone" ? "Welcome to Sells Point" : "Verify your number"}
            </h2>
            <p className="text-sm text-ink-500">
              {step === "phone"
                ? "Sign in with your phone number to buy & sell"
        ...
        {step === "phone" && (
          <form onSubmit={handleSendOtp} className="space-y-4">
            ...
            <div>
              <label className="mb-1.5 block text-sm font-medium text-ink-700">
                Name <span className="text-ink-400">(for new accounts)</span>
              </label>
              <div className="relative">
        ...
        {step === "otp" && (
          <form onSubmit={handleVerify} className="space-y-4">
            <div className="rounded-xl bg-brand-50 px-4 py-3 text-sm text-brand-700">
              Demo mode: use code <span className="font-bold">{sentCode}</span>
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-ink-700">
                Enter 6-digit OTP
